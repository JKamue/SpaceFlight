...
namespace SpaceFlight.Screen.Calculator
{
    class ProjectedPositionCalculator
    {
        ...
        public Point ProjectPoint(PointD p) => new Point(ProjectXCoordinate(p.X), ProjectYCoordinate(p.Y));

        public int ProjectXCoordinate(decimal x) => RoundToInt((_pMP.X + (x - (decimal)_rMP.X)) * (decimal)_factor);
        ...
        public int ProjectYCoordinate(decimal y) => RoundToInt((_pMP.Y + (y - (decimal)_rMP.Y) * -1) * (decimal)_factor);
        ...
        public int RoundToInt(decimal f) => (int)Math.Round(f);
    }
}